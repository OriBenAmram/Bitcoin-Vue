<template>
    ...
    <section v-if="isAdding" class="add-contact-section">
        <form class="inputs-container" @submit.prevent="onSaveContact">
            ...
            <input ref="email" required type="text" placeholder="Contact mail..." v-model="contact.email"
                class="secondary-input">
            <input ref="phone" required type="text" placeholder="Contact phone..." v-model="contact.phone"
                class="secondary-input">
...
export default {
    ...
    methods: {
        ...
        onTransfer() {
            ...
            this.amount = "";
            const userMsg = { header: 'Thank you', txt: `You have just transfered ${randomContact.name} a 100$. You have probably made their day a bit better!` }
            this.$store.dispatch({ type: "onSetUserMsg", userMsg });
        },
    },
    mounted() {
